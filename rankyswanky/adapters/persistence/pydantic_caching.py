"""
Read pydantic models and stores them in SQLite database using SQLModel.
...
def get_sqlmodel_by_primary_key(
    model: Type[SQLModel], primary_key_value: str, db_path: str = "documents.db"
) -> SQLModel | None:
    """Retrieve a SQLModel instance by its ID from the SQLite database."""
    engine = create_engine(f"sqlite:///{db_path}")
    with Session(engine) as session:
